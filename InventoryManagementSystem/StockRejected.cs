...
{
    public partial class StockRejected : Sample2
    {
       
        Retrievel retrieve = new Retrievel();
      
        public StockRejected()
        {
            InitializeComponent();
        ...
        {
            retrieve.ShowRejectedStock(dataGridViewProduct, ProductIdGv, ProductNameGv, ProductCodeGv, ProductUnitGv, ProductMinValGv, AvailableStocks,ReworkedStock,RejectedStock,OnholdStock);
        ...
        public override void textBoxSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                retrieve.ShowRejectedStock(dataGridViewProduct, ProductIdGv, ProductNameGv, ProductCodeGv, ProductUnitGv, ProductMinValGv, AvailableStocks,ReworkedStock,RejectedStock,OnholdStock, textBoxSearch.Text);
                //foreach (DataGridViewRow rows in dataGridViewProduct.Rows)
                //{
                //    if (Convert.ToDecimal(rows.Cells["ProductMinValGv"].Value) < Convert.ToDecimal(rows.Cells["AvailableStocks"].Value))
                //    {
