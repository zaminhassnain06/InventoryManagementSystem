...
{
    public partial class StockOnHold : Sample2
    {
        ...
        {
            retrieve.ShowOnHoldStock(dataGridViewProduct, ProductIdGv, ProductNameGv, ProductCodeGv, ProductUnitGv, ProductMinValGv, AvailableStocks, ReworkedStock, RejectedStock, OnholdStock);
        }
        ...
        public override void textBoxSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                retrieve.ShowOnHoldStock(dataGridViewProduct, ProductIdGv, ProductNameGv, ProductCodeGv, ProductUnitGv, ProductMinValGv, AvailableStocks,ReworkedStock,RejectedStock,OnholdStock ,textBoxSearch.Text);
                //foreach (DataGridViewRow rows in dataGridViewProduct.Rows)
                //{
                //    if (Convert.ToDecimal(rows.Cells["ProductMinValGv"].Value) < Convert.ToDecimal(rows.Cells["AvailableStocks"].Value))
                //    {
        ...
        private void buttonResolveOnHoldStock_Click(object sender, EventArgs e)
        {
            if (comboBoxSelectProduct.SelectedIndex == -1)
            {
                labelProductDateError.Visible = true;
            ...
            else
            {
                ...
                int row = update.UpdateResolveInOnHoldStocks(Convert.ToInt32(comboBoxSelectProduct.SelectedValue.ToString()), Convert.ToDecimal(textBoxQuantity.Text), dateTimePickerEntryDate.Value);
                ...
                {
                    MainClass.ShowMessage("Please Add Stocks in On Hold First.", "Error", "Error");
                }
